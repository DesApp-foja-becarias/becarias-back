...
export const getStatusAcademicoSimplificado = async (becariaId) => {
  return await axios
...
export const getCarrerasDeBecaria = async (
  becariaDni,
  carrerasDelDepto = null
) => {
  //retornara un array de carreras con las que esta becaria esta relacionada y sus materias
  let carreras;
  if (carrerasDelDepto == null) {
    await getCarrerasDeDeptoDeIngenieria().then((response) => {
      carreras = response.data.carreras;
    });
  } else {
    carreras = carrerasDelDepto;
  }
  ...
  let ACADEMIC_STATUS;
  if (carrerasDelDepto == null) {
    ACADEMIC_STATUS = await Promise.all(
    ...
    ACADEMIC_STATUS = await Promise.all(
      BECARIA_IDS_FILTERED.map(async (id) => {
        const academicStatus = await getStatusAcademicoSimplificado(id);
        return {
          ...academicStatus,
        };
      })
